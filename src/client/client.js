...
export default async () => {
  ...
  inputPort.on('drain', () => console.log(`DEBUG  (${getTimestamp()}): input port drain`)); // TODO: Try to trigger
  ...
  console.log(`DEBUG  (${getTimestamp()}): Authenticated with token ${authToken}`);
  ...
  parserInput.on('data', async data => {
    ...
    console.log(`INPUT  (${getTimestamp()}): 0x${data.toString('hex').toUpperCase()}`);

    if (!authToken) {
      authToken = await authenticate();
      console.log(`DEBUG  (${getTimestamp()}): RE-Authenticated with token ${authToken}`);
    }
    ...
    if (checksum.check(data) === 0) {
      switch (command) {
        case COMMANDS.READ:
          ...
          console.log(`DEBUG  (${getTimestamp()}): CardId ${cardId} with balance ${balance} = ${balance.toString(16)} HEX`);
          message = Buffer.from(`${BYTES.START}${COMMANDS.READ}${cardId}${balance.toString(16)}`);
          generatedChecksum = checksum.generate(message).toString(16);
          console.log(`DEBUG  (${getTimestamp()}): read ${message}${generatedChecksum}`);
          outputPort.write(`${message}${generatedChecksum}`);
          break;
        case COMMANDS.ADD:
          balanceAfterDeposit = await deposit({ amount: credits, authToken, cardId });
          console.log(`DEBUG  (${getTimestamp()}): CardId ${cardId} balance after deposit ${balanceAfterDeposit}`);
          break;
        case COMMANDS.DECREASE:
          balanceAfterWithdraw = await withdraw({ amount: credits, authToken, cardId });
          console.log(`DEBUG  (${getTimestamp()}): CardId ${cardId} balance after withdraw ${balanceAfterWithdraw}`);
          break;
      ...
      console.log(`DEBUG  (${getTimestamp()}): Message 0x${data.toString('hex').toUpperCase()} has wrong checksum`);
  ...
  parserOutput.on('data', async data => {
    console.log(`OUTPUT (${getTimestamp()}): 0x${data.toString('hex').toUpperCase()}`);
  });
  ...
  const runTestCommands = parser => {
    ...
    setTimeout(() => parser.write(Buffer.from('AB5602017BAF1F680064186A', 'hex')), 500); // Add
    setTimeout(() => parser.write(Buffer.from('AB5603017BAF1F680001198D', 'hex')), 1000); // Decrease
    setTimeout(() => parser.write(Buffer.from('AB560100000000110000D130', 'hex')), 1500); // Message with wrong checksum
